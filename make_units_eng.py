...
tdb_units = {
    str(msid.upper()): str(unit)  # convert from numpy type to pure Python
    for msid, unit in zip(dat["MSID"], dat["ENG_UNIT"])
}
units_eng.update(
    {msid: tdb_units[msid] for msid, unit in units_eng.items() if msid in tdb_units}
)

# Any other MSIDs that still have units of 'K' are converted to DEGC
units_eng.update(
    {msid: "DEGC" for msid, unit in units_eng.items() if unit in ["K", "deltaK"]}
)
