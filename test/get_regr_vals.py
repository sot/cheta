...
def get_options():
    ...
    parser.add_option("--start", default="2024:005", help="Start time")
    parser.add_option("--stop", default="2024:012", help="Stop time")
    parser.add_option("--out", default="regr_vals", help="Output prefix")
...
if opt.test:
    sys.path.insert(0, "..")
    if "ENG_ARCHIVE" not in os.environ:
        os.environ["ENG_ARCHIVE"] = os.path.abspath(os.getcwd() + "/eng_archive")
    outfile = opt.out + ".test.pkl"
else:
    outfile = opt.out + ".flight.pkl"
...
for msid in msids:
    ...
    out[msid] = dict(
        dat=dict((x, getattr(dat, x)) for x in attrs if hasattr(dat, x)),
...
pickle.dump(out, open(outfile, "wb"), protocol=-1)
